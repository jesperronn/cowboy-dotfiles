...
function __prompt_command() {
  ...
  # date printed right side:
  # PS1="$PS1\[$(tput sc; __prompt_timestamp; tput rc)\]"
  # date: [HH:MM:SS] printed with \t

  compensate=6 #compensate width for color codes used (in '$c1[\t]')
  right_col=$(($(tput cols)+$compensate))
  PS1="$PS1$( printf "%*s\r$sc9" "$right_col" "$c1[\t]" )"
